...
const { isString, toArray, isValue, isFunction, isBoolean, isPlainObject, isArray, isRegExp, toRegExp, isDate } = require('chek');
...
function replaceAt(str, idx, val) {
  if (typeof idx === 'string')
    idx = str.indexOf(idx);
  str = str.slice(0, idx) + val + str.slice(idx + val.length);
  return str;
}

function replaceInsert(str, start, end, val) {
  if (typeof start === 'string')
    start = str.indexOf(start);
  if (typeof end === 'string')
    end = str.lastIndexOf(end);
  str = str.slice(0, start + 1) + val + str.slice(end);
  return str;
}

function replaceRange(str, start, end, val) {
  if (typeof start === 'string')
    start = str.indexOf(start);
  if (typeof end === 'string')
    end = str.lastIndexOf(end);
  str = str.slice(0, start) + val + str.slice(end + 1);
  return str;
}
