...
function renameList(dir, map, preview, options) {
  ...
  if (!fs.existsSync(csvPath)) {
    utils.log(`cannot restore from directory ${utils.toRelative(csvPath, dir)} the path was not found.`, 'ERROR', 'red');
    process.exit(1);
  }
  ...
  if (preview) {
    ...
    if (matches.length) {
      ...
      const prevTableStr = table(prevTable, {
        stringLength: (s) => {
    ...
    if (filesClone.length) {
      ...
      const confTableStr = table(confTable, {
        align: ['l'],
        stringLength: (s) => {
          return colurs.strip(s).length;
        }
