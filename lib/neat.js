...
function replaceList(dir, options) {
  ...
  const files = utils.walk(dir, (path, current) => {
    return transformFilename(path, current, options, false);
  });

  files.forEach((o) => {

    if (find.test(o.filename)) {

      if (options.all && matches.length > 0) {
        utils.log('Nice try! Cannot replace complete filename of multiple files.', 'ERROR', 'red');
        process.exit(1);
      }
